...
extern double** calculatePinv(double** n, int row, int col);
...
{
    int i,j;
    number_weight = 275;//todo:remember to change number_weight
    lambda = (double **) malloc (number_weight * sizeof(double *));
    for (i = 0; i < number_weight; i++)
        lambda[i] = (double *) malloc(number_objective * sizeof(double));
    FILE *fp;
    if ((fp = fopen ("m8.txt","r"))==NULL)//todo:change the txt file
    {
        printf("error\n");
        exit(1);
    }
...
void rbf_train(double **a,double *rbf,double *weights_rbf,double sigma)
{
    ...
    double *center;
    center = (double *)malloc(number_objective * sizeof(double));//10 RBF centers


     for(i = 0; i < 10; i++)
    {
         for(j = 0;j < 10;j++)
        active_function[i][j] = 0;//RBF outputs
    }
     for (i = 0;i < 10;i++)//control RBF
    {
        for (j = 0; j < number_objective; j++)
            center[j] = a[i][j];//set RBF center equal to training data

        for (j = 0; j < 10; j++)//control training data
        {
     ...
     /*active_function denotes RBF output: every col is every training data 's 10 RBF results*/
    double** pinv_active_function;
    pinv_active_function = calculatePinv(active_function,10,10);

       for(i = 0; i < 10; i++)
       {
           weights_rbf[i]=0.0;
           for(j = 0;j < 10;j++)
              weights_rbf[i] += rbf[j] * pinv_active_function[i][j];
       }
    double predicted;
    for (int m = 0; m < 10; ++m)
    {
        ...
        for (int l = 0; l < 10; ++l)
        {
            predicted += weights_rbf[l] * active_function[l][m];
        }
        printf("true:%f\tpredicted:%f\n",rbf[m],predicted);
    }
}
...
/*ask DM to score in the first consultation*/
void rbf_first_train(double **a,double *rbf,double *weights_rbf,double sigma,int miu)
{
    ...
    double** pinv_active_function;
    pinv_active_function = calculatePinv(active_function,miu,miu);
    for(i = 0; i < miu; i++)
    {
        weights_rbf[i]=0.0;
        for(j = 0;j < miu;j++)
