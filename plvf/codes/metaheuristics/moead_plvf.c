...
/* common paramters */
   ...
   nums_weight();
    ...
    printf("umber of weight vectors=%d\tpopulation size=%d\n",number_weight,popsize);
    ...
    // todo:initialize parameter settings
...
//    eta = 0.3;
        ...
        if (generation == first_tau)
        {
           ...
           // set miu "seed" reference points;
            ...
            for(i = 0; i < miu; i++)
            {
                ...
                /*todo:change the way of scoring of the DM*/
    ...
    printf("preference solutions:\n");
    for (int l = 0; l < number_weight; ++l)
    {
        for (int m = 0; m < number_objective; ++m)
        {
            printf("%f\t",pop->ind[l].obj[m]);
        }
        printf("\n");
    }
    free (permutation);
